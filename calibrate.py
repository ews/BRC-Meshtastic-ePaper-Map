...
<div id="sidebar">
  ...
  <button onclick="saveToConfig()" class="green" style="width:100%;">💾 Save to config.yaml</button>
...
function saveToConfig() {
  ...
  ANCHORS.forEach((a, i) => {
    const px = pixelCoords[i];
    anchorsOut.push([a[0], a[1], px.set ? px.x : 0, px.set ? px.y : 0]);
  });

  fetch('/api/save-anchors', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({anchors: anchorsOut})
  })
  .then(r => r.json())
  .then(data => {
    document.getElementById('status').textContent = '✅ Saved ' + anchorsOut.length + ' anchors to config.yaml';
  })
  .catch(e => {
...
def _save_anchors_to_config(anchors):
    """Write anchor pixel positions back to config.yaml."""
    import os
    ...
    try:
        ...
        print(f"[calibrate] ERROR reading config: {e}")
        return

    # Update only the pixel positions (keep GPS coords from existing anchors)
    existing = cfg.get("anchors", [])
    updated = []
    for i, new_a in enumerate(anchors):
        lat = new_a[0]
        lon = new_a[1]
        px = new_a[2]
        py = new_a[3]
        # Try to match existing anchor by GPS proximity
        matched = False
        for old_a in existing:
            if abs(old_a[0] - lat) < 0.001 and abs(old_a[1] - lon) < 0.001:
                updated.append([lat, lon, px, py])
                matched = True
                break
        if not matched:
            updated.append([lat, lon, px, py])

    # Add any anchors from existing that weren't updated (keep their values)
    for old_a in existing:
        if not any(
            abs(u[0] - old_a[0]) < 0.001 and abs(u[1] - old_a[1]) < 0.001
            for u in updated
        ):
            updated.append(old_a)
    ...
    try:
        ...
        print(f"[calibrate] Saved {len(updated)} anchors to {CONFIG_PATH}")
    except OSError as e:
        print(f"[calibrate] ERROR writing config: {e}")
        ...
        def do_POST(self):
            if self.path == "/api/save-anchors":
                try:
                    ...
                    _save_anchors_to_config(anchors)
                    self.send_response(200)
